...
(() => {
  ...
  if(embedded)return;
  // Contacts must remain available when the shared navigation endpoint is unavailable.
  if(!document.querySelector('script[data-imkonex-contacts]')){
    const contacts=document.createElement('script');contacts.src=base+'/shared/contact-widget.js';
    contacts.dataset.imkonexContacts='true';contacts.dataset.origin=base;document.head.append(contacts);
  }
  async function mount(){
    ...
    if(Array.isArray(data.contacts)){
      window.IMKONEX_SITE_CONTACTS=data.contacts;
      document.dispatchEvent(new CustomEvent('imkonex:contacts',{detail:data.contacts}));
    }
    ...
    for(const kind of ['header','footer']){
      ...
      // The shared corporate shell must lead buyers to this store's seller and policies.
      for(const link of template.content.querySelectorAll('a[href]')){
        try{
          const url=new URL(link.getAttribute('href'),base);
          if(!['www.imkonex.com','imkonex.com'].includes(url.hostname))continue;
          const local={'/kontakty':'/info/contacts/','/polzovatelskoe-soglashenie':'/info/terms/'}[url.pathname.replace(/\/$/,'')];
          if(local){link.href=base+local;link.removeAttribute('target');}
        }catch{}
      }
      ...
      const close=()=>{header?.classList.remove('is-menu-open');burger?.setAttribute('aria-expanded','false');document.body.style.overflow='';document.dispatchEvent(new Event('imkonex:shell-menu'));};
      burger?.addEventListener('click',()=>{const open=header.classList.toggle('is-menu-open');burger.setAttribute('aria-expanded',String(open));document.body.style.overflow=open?'hidden':'';document.dispatchEvent(new Event('imkonex:shell-menu'));});
