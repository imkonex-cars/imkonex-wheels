...
ROOT_FILES = ('BUILD_RENDER.mjs', 'package.json', 'package-lock.json', 'requirements.txt',
              ...
              'render-api.yaml', 'render-manager.yaml', 'Dockerfile.portal', '.dockerignore', 'UPGRADE_v0.5.0.txt', 'README.md', 'RELEASE_NOTES.md', 'START_HERE_PC1.txt', 'SYNC_CATALOG_WORKFLOW.txt', '.env.example', '.gitignore')
...
def main():
    ...
    manifest = (f'IMKONEX CARS — полная рабочая область {version}\n'
                ...
                'REG.ru: Dockerfile.portal, приватная БД на постоянном томе. Обновление — по START_HERE.md полного пакета.\n'
