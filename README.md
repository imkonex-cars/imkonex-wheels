...
После обновления PHOTOS-1 все шесть фотографий входят в проект: frontend/assets/products/. Это исходные изображения по полю img_big_pish из отчёта поставщика; они скачаны и проверены 21.09.2026. Браузер загружает их с того же сайта, что и каталог. Адреса источников и контрольные суммы находятся в docs/PRODUCT_PHOTO_SOURCES.json. Сборка выводит Photos: 6 local, 0 remote, 0 unavailable. При повреждении или отсутствии изображения в браузере остаётся нейтральная заглушка. Данные о ценах и остатках относятся к прежней дате 17.09.2026. Обновление снимка через экспорт API потребует отдельно подготовить изображения для новых товаров.
